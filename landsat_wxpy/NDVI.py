...
class NDVI:
    ...
    def __init__(self):
        ...
        self.listener = OnProcessFinishListener
    ...
    def SetCropCoordinate(self, lonStart, lonEnd, latStart, latEnd):
        ...
        if issue:
            ...
            for item in issue:
                issueBuf += item + "\n"
                print(issueBuf)
            self.listener.showMessageError(self, "Potong Citra Gagal\n\n" + issueBuf)
    ...
    def OpenMtlFile(self, path):
        ...
        if "mtl" in path.lower():
            ...
            if issue:
                ...
                for item in issue:
                    issueBuf += item + "\n"
                self.listener.showMessageError(self, "Import Metadata Gagal\n\n" + issueBuf + "\n" + "Penyebabnya Adalah Salah Memilih File Metadata")
                self.isMtl = False
            else:
                # self.listener.showMessage(self, "Successfully import mtl file")
                self.isMtl = True
        else:
            self.listener.showMessageError(self, "File Text Salah Dipilih")
    ...
    def CropImage(self):
        ...
        #     self.listener.onCropFinish(self, self.crop_B4, self.crop_B5)
...
class OnProcessFinishListener:
    ...
    def showMessage(self, message):
        wx.MessageBox(message, "Pesan NDVI", wx.OK | wx.ICON_INFORMATION)

    def showMessageError(self, message):
        wx.MessageBox(message, "Error NDVI", wx.OK | wx.ICON_ERROR)